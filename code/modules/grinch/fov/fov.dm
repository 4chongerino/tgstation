...
atom/movable
	...
	proc/ShowMask(client/c) // Create a mask if it doesn't exist, making the object invisible to a certain client
		...
		if(!vmask)
			vmask = new/vmask(src, c)

		c.AddMask(vmask.mask_image)

image/vmask

vmask

	var/image/vmask/mask_image  // a unique path to make synching client.images easier
...
client
    ...
    proc/SyncMasks()  // Synchs the masks list with images list and cleans up un-needed masks
        images.Remove(masks)
        for(var/image/vmask/i in images)
            images.Remove(i)
        images.Add(masks)
